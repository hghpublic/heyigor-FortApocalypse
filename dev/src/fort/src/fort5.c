...
/* From fnt1.h: constants — POS_MASK1; data — FORT_EX1, FORT_EX2, FORT_EX3,
 * FORT_EX4. */

/* From fort1.h: variables — adr1_lo, adr1_hi, adr2_lo, adr2_hi, temp1-6, mode,
 * level, fort_status, laser_status, fuel_status, fuel1, fuel2, fuel_temp,
 * bonus1, bonus2, chopper_status, chop_x, chop_y, sx, sy, bak_color,
 * bak2_color, robot_x, slave_status[8], slave_x[8], slave_y[8], slave_dx[8],
 * slave_num, slaves_left, slaves_saved, rocket_x[3], tim4_val, tim9_val,
 * s1_1_val, s1_2_val, s2_val, s3_val, s4_val, s5_val, s6_val, scan_adr1_lo,
 * scan_adr1_hi, scan_adr2_lo, scan_adr2_hi, land_x, land_y, land_fx, land_fy,
 * land_chop_x, land_chop_y, land_chop_angle.
 * Functions — compute_map_adr, save_pos, give_bonus, clear_info, clear_sounds,
 * wait_frame, print, inc_score, do_sounds. */
 ...
 *   Aliases:    TILE_EXP=CHR_EXP, TILE_MISS_L=CHR_MISS_LEFT,
 * TILE_MISS_R=CHR_MISS_RIGHT HW aliases: REG(); FRAME_HW=FRAME,
 * RANDOM_HW=RANDOM, AUDF1_HW..AUDF4_HW=AUDF1..AUDF4,
 * AUDC1_HW..AUDC4_HW=AUDC1..AUDC4, HPOSP0_HW=HPOSP0, HPOSP2_HW=HPOSP2,
 * HPOSP3_HW=HPOSP3, COLBK_HW=COLBK, COLPF0_HW..COLPF3_HW=COLPF0..COLPF3,
...
static const uint8_t slave_pickup_mess[] = {0xCD, 0xC5, 0xCE, /* MEN */
                                            0x20, 0x20,       /*     */
                                            0xD4, 0xCF,       /* TO  */
                                            0x20, 0x20,       /*     */
                                            0xD2, 0xC5, 0xD3,
                                            0xC3, 0xD5, 0xC5, /* RESCUE */
                                            0xFF};

/* String: "LOW  ON  FUEL" (inverted) + 0xFF terminator */
static const uint8_t warning_msg[] = {0xCC, 0xCF, 0xD7,       /* LOW  */
                                      0x20, 0x20, 0xCF, 0xCE, /* ON   */
                                      0x20, 0x20, 0xC6, 0xD5,
                                      0xC5, 0xCC, /* FUEL */
                                      0xFF};
 ...
 * character cell).  Updates scan_adr1/adr1 (+40 per row) and scan_adr2/adr2
 * (+1).
 */
static void do_line(void)
{
    for (int8_t row = 7; row >= 0; row--)
    {
        ...
        uint16_t sa1 =
            (uint16_t)((uint16_t)scan_adr1_hi << 8 | scan_adr1_lo) + 40u;
        scan_adr1_lo = (uint8_t)(sa1 & 0xFFu);
        scan_adr1_hi = (uint8_t)(sa1 >> 8u);
        adr1_lo = scan_adr1_lo;
        adr1_hi = scan_adr1_hi;
        uint16_t sa2 =
            (uint16_t)((uint16_t)scan_adr2_hi << 8 | scan_adr2_lo) + 1u;
...
static void next_part1(void)
{
    ...
    fort_exp[0] = (const uint8_t*)FORT_EX1;
    fort_exp[1] = (const uint8_t*)FORT_EX2;
    fort_exp[2] = (const uint8_t*)FORT_EX3;
    fort_exp[3] = (const uint8_t*)FORT_EX4;
    ...
    while (temp3 < 4u)
    {
        ...
        for (temp4 = 0u; temp4 < 6u; temp4++)
        {
            ...
            {
                const uint8_t* fd =
                    (const uint8_t*)(uintptr_t)((uint16_t)adr2_hi << 8 |
                                                adr2_lo);
                ...
                uint8_t t5 = fd[temp4];
                uint8_t* row_base =
                    (uint8_t*)(uintptr_t)((uint16_t)adr1_hi << 8 | adr1_lo);
...
static int s_col(uint8_t idx)
{
    ...
    const uint8_t* bot =
        (const uint8_t*)(uintptr_t)((uint16_t)adr1_hi << 8 | adr1_lo);
    uint8_t tile = *bot;
    if (tile == 0u || tile == TILE_EXP || tile == TILE_MISS_L ||
        tile == TILE_MISS_R)
    {
        return s_col2(idx);
    }
...
void set_scanner(void)
{
    ...
    /* Base address = SCANNER + (sx>>3) + (temp2:temp1) = $39C0 + col +
     * row_offset */
    uint32_t addr = (uint32_t)0x39C0u + (uint32_t)(sx >> 3u) + (uint32_t)temp1 +
                    ((uint32_t)temp2 << 8u);
...
void pos_it(void)
{
    ...
    *p ^= ((const uint8_t*)POS_MASK1)[tx & 7u];
}

void check_fort(void)
{
    if (fort_status == STATUS_EXPLODE)
    {
        do_checksum1();
    }
}
...
/* -----------------------------------------------------------------------
 * Display-list interrupt handlers (chained: LINE1 → LINE2 → LINE3 → LINE4 →
 * LINE1)
 * ----------------------------------------------------------------------- */
...
void line4(void)
{
    ...
    WSYNC_HW = 0u;
    COLBK_HW = 0u;
    if (mode == STOP_MODE || mode == GO_MODE)
    {
        do_sounds();
    }
}
...
void do_sounds(void)
{
    /* S1: chopper engine (every other 2-frame; freq descends from
     * s1_1/s1_2_val) */
