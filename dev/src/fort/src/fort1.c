...
#define SET_SDLST(p)                                                           \
    do                                                                         \
    {                                                                          \
        SDLST_L = (uint8_t)((uintptr_t)(p) & 0xFF);                            \
        SDLST_H = (uint8_t)((uintptr_t)(p) >> 8);                              \
    } while (0)
...
/* T.1: "FORT  APOCALYPSE" */
static const uint8_t T_1[] = {S('F'), S('O'), S('R'), S('T'), 0x20,   0x20,
                              S('A'), S('P'), S('O'), S('C'), S('A'), S('L'),
                              S('Y'), S('P'), S('S'), S('E'), 0xFF};

/* T.2: "BY  STEVE  HALES" */
static const uint8_t T_2[] = {S('B'), S('Y'), 0x20,   0x20,   S('S'), S('T'),
                              S('E'), S('V'), S('E'), 0x20,   0x20,   S('H'),
                              S('A'), S('L'), S('E'), S('S'), 0xFF};

/* T.3: "COPYRIGHT" */
static const uint8_t T_3[] = {S('C'), S('O'), S('P'), S('Y'), S('R'),
                              S('I'), S('G'), S('H'), S('T'), 0xFF};

/* T.4: "SYNAPSE  SOFTWARE" */
static const uint8_t T_4[] = {S('S'), S('Y'), S('N'), S('A'), S('P'), S('S'),
                              S('E'), 0x20,   0x20,   S('S'), S('O'), S('F'),
                              S('T'), S('W'), S('A'), S('R'), S('E'), 0xFF};

/* NEW.PILOT: "GET  READY  PILOT" */
static const uint8_t new_pilot[] = {
    S('G'), S('E'), S('T'), 0x20,   0x20,   S('R'), S('E'), S('A'), S('D'),
    S('Y'), 0x20,   0x20,   S('P'), S('I'), S('L'), S('O'), S('T'), 0xFF};

/* PILOTS.LEFT: "PILOTS  LEFT" */
static const uint8_t pilots_left[] = {S('P'), S('I'), S('L'), S('O'), S('T'),
                                      S('S'), 0x20,   0x20,   S('L'), S('E'),
                                      S('F'), S('T'), 0xFF};

/* ENTER: "ENTERING" */
static const uint8_t str_enter[] = {S('E'), S('N'), S('T'), S('E'), S('R'),
                                    S('I'), S('N'), S('G'), 0xFF};

/* LVL.1: "VAULTS  OF  DRACONIS" */
static const uint8_t lvl_1[] = {S('V'), S('A'), S('U'), S('L'), S('T'), S('S'),
                                0x20,   0x20,   S('O'), S('F'), 0x20,   0x20,
                                S('D'), S('R'), S('A'), S('C'), S('O'), S('N'),
                                S('I'), S('S'), 0xFF};

/* LVL.2: "CRYSTALLINE  CAVES" */
static const uint8_t lvl_2[] = {S('C'), S('R'), S('Y'), S('S'), S('T'),
                                S('A'), S('L'), S('L'), S('I'), S('N'),
                                S('E'), 0x20,   0x20,   S('C'), S('A'),
                                S('V'), S('E'), S('S'), 0xFF};
...
/* G.C: "COMPLETED" */
static const uint8_t G_C[] = {'C', 'O', 'M', 'P', 'L',
                              'E', 'T', 'E', 'D', 0xFF};

/* G.2: "YOUR  RANK  IS" */
static const uint8_t G_2[] = {'Y', 'O', 'U',  'R',  0x20, 0x20, 'R', 'A',
                              'N', 'K', 0x20, 0x20, 'I',  'S',  0xFF};

/* G.3: "CLASS" (.AT -) */
static const uint8_t G_3[] = {S('C'), S('L'), S('A'), S('S'), S('S'), 0xFF};

/* Rating strings (.AT -) */
static const uint8_t R_1[] = {S('S'), S('P'), S('A'), S('R'),
                              S('R'), S('O'), S('W'), 0xFF};
static const uint8_t R_2[] = {S('C'), S('O'), S('N'), S('D'),
                              S('O'), S('R'), 0xFF};
static const uint8_t R_3[] = {S('H'), S('A'), S('W'), S('K'), 0xFF};
static const uint8_t R_4[] = {S('E'), S('A'), S('G'), S('L'), S('E'), 0xFF};

/* HS: "HIGH  SCORE" */
static const uint8_t HS[] = {S('H'), S('I'), S('G'), S('H'), 0x20,   0x20,
                             S('S'), S('C'), S('O'), S('R'), S('E'), 0xFF};
...
static const uint8_t level_chop_start[3][2] = {
    {90, 100}, {119, 100}, {116, 170}};
static const uint8_t level_start[3][2] = {
    {0x02, 0xFF}, {0x6D, 0xFF}, {0x6E, 0x18}};
...
static const uint8_t tank_start_x_l1[MAX_TANKS] = {0x53, 0x63, 0x90,
                                                   0xA0, 0x59, 0xAE};
static const uint8_t tank_start_y_l1[MAX_TANKS] = {0x12, 0x12, 0x12,
                                                   0x12, 0x26, 0x26};
static const uint8_t tank_start_x_l2[MAX_TANKS] = {0x50, 0x65, 0xA0,
                                                   0xB5, 0x3D, 0x54};
static const uint8_t tank_start_y_l2[MAX_TANKS] = {0x0C, 0x0C, 0x0C,
                                                   0x0C, 0x26, 0x26};
...
static const uint8_t chr1[CHR1_LEN] = {
    /* .AT " a./01*+,-#'?st" */
    0x20, 0x61, 0x2E, 0x2F, 0x30, 0x31, 0x2A, 0x2B, 0x2C, 0x2D, 0x23, 0x27,
    0x3F, 0x73, 0x74,
    /* .HS 41444858595AD8 */
...
void t2_vblank(void)
{
    ...
    {
        demo_status =
            (uint8_t)-1; /* MAIN loop will set mode=START_MODE next frame */
        t3_game_start();
        return;
    }
    ...
    if (TRIG0 != 0)
    { /* trigger NOT pressed (TRIG0==0 = pressed) */
        uint8_t con = CONSOL;
        if (con != 6)
        { /* START button not pressed */
            if (con == 7)
            {
                return; /* no console buttons: do nothing */
            }
...
void m_new_player(void)
{
    ...
    if ((chop_left & 0x0F) == 0)
    {
        ...
        {
            chop_left =
                (uint8_t)((chop_left - 0x10) | 0x09); /* borrow from tens */
        }
    }
    else
    {
...
void m_game_over(void)
{
    ...
    /* Write rank numeral and letter to screen (ADR1 set by preceding PRINT) */
    {
        ...
        uint8_t rank_num =
            (uint8_t)(((game_points & 3) ^ 3) + 1); /* 1=lowest, 4=highest */
