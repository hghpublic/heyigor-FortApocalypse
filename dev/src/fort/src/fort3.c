...
/* -----------------------------------------------------------------------
 * From fort1.h:
 * STATUS_* constants: OFF, ON, LAND, PICKUP, FLY, CRASH, BEGIN, EMPTY, REFUEL,
 * EXPLODE Mode constants: GO_MODE, HYPERSPACE_MODE, NEW_PLAYER_MODE Constant:
 * HIT_LIST_LEN External variables: mode, level, chopper_status, chopper_x,
 * chopper_y, chopper_angle, chopper_col, robot_status, robot_x, robot_y,
 * robot_angle, robot_col, robot_spd, r_status, r_fx, r_fy, r_x, r_y, sx, sy,
 * sx_f, sy_f, chop_x, chop_y, ochopper_y, orobot_y, orocket_y[],
 *   rocket_status[], rocket_x[], rocket_y[], rocket_tim[], rocket_temp[],
 *   rocket_tempx[], rocket_tempy[], tim3_val, tim7_val, tim8_val,
 *   fuel_status, fort_status, grav_skl, ssizem, s2_val, s3_val, s5_val,
 * bak2_color, land_fx, land_fy, land_x, land_y, land_chop_x, land_chop_y,
 * land_chop_angle, chopper_shapes[18] External functions: pos_chopper(),
 * pos_robot(), compute_map_adr_i(), pick_up_slave(), inc_score(), do_numbers(),
 * draw_map(), read_trig(), do_exp(), do_laser_1(), do_laser_2(), do_blocks(),
 * do_elevator(), read_stick()
 * ----------------------------------------------------------------------- */
...
/* fort5.c — returns 1 if a slave was picked up (assembly: carry set), 0
 * otherwise */
...
static void check_land(void)
{
    ...
    for (int i = LAND_LEN; i >= 0; i--)
    {
        if (b == land_chr[i])
        {
            return; /* landing tile: OK, no increment */
        }
        if (b == 0x48u)
        {
            temp4_i++;
...
void robot_brains(void)
{
    ...
    if (st == STATUS_OFF)
    {
        ...
        if (level == 1u)
        {
            idx = (uint8_t)(idx + 8u); /* level 1: second spawn set */
        }
        ...
        if ((int8_t)dx < 0)
        {
            dx ^= 0xFEu; /* EOR #-2 approximate abs */
        }
        if (dx < 34u)
        {
            ...
            if (dy < 8u)
            {
                return; /* too close: abort spawn, timer wraps to 255 */
            }
        }
    ...
    if (--tim8_val == 0u)
    {
        ...
        if (robot_status == STATUS_ON && rocket_status[2] == 0u)
        {
            ...
            static const uint8_t fire_dir[9] = {1u, 1u, 2u, 3u, 3u,
                                                3u, 4u, 5u, 5u};
    ...
    /* R.END: clamp ROBOT.ANGLE to [0,17], restore oscillation bit, mask
     * fractions */
    {
        uint8_t ang = robot_angle;
        if ((int8_t)ang < 0)
...
void do_chopper(void)
{
    ...
    {
        if (pick_up_slave())
        { /* returns 1 if slave picked up (carry) */
...
void do_robot_chopper(void)
{
    ...
    /* Compute pixel Y: (R.Y - max(SY,0))*8 + 83 + (~SY.F & 7) + [SY<0:+8] +
     * R.FY */
    {
        uint8_t sy_clamp = ((int8_t)sy < 0) ? 0u : sy;
...
static void rocket_exp(void)
{
    for (int x = 2; x >= 0; x--)
    {
        if (rocket_status[x] != 7u)
        {
            continue; /* not exploding */
        }
        if (--rocket_tim[x] != 0u)
        {
...
void update_rockets(void)
{
    for (int x = 2; x >= 0; x--)
    {
        ...
        if (status != 0u && status != 7u)
        {
            ...
            /* Y: rocket_y + (sy>=0 ? sy_f&7 : 0) - 94, divided by 8, +
             * max(sy,0) */
            uint8_t sy_fine = ((int8_t)sy >= 0) ? (uint8_t)(sy_f & 7u) : 0u;
            uint8_t my_raw =
                (uint8_t)((uint8_t)(rocket_y[x] + sy_fine - 94u) >> 3);
            uint8_t sy_base = ((int8_t)sy >= 0) ? sy : 0u;
            ...
            if (check_chr_i())
            {
                ...
                if (tile == 0xC7u)
                {
                    /* $C7 = EXP.WALL: score, place 0 as saved tile (destroys
                     * wall) */
                    bak2_color = 0x10u;
                    uint8_t tmpx = (uint8_t)x;
                    inc_score(0x20u, 0x00u);
                    rocket_temp[tmpx] =
                        0u; /* saved=0: rocket_exp writes 0 (clears) */
                    new_status = 7u;
                    goto rocket_col_done;
                }
            ...
            rocket_col_done:
                ...
                adr1_i_ptr()[0] =
                    0x20u; /* place EXP character at hit location */
                rocket_tim[x] = 7u;
            }
        }
        ...
        else if (status != 7u)
        {
            ...
            if (rx < 4u || rx >= 251u || ry >= (uint8_t)(212u + 18u) ||
                ry < 100u)
            {
                rocket_status[x] = 0u;
                clear_pos = 1;
        ...
        {
            rocket_x[x] =
                (uint8_t)((int8_t)rocket_x[x] + rocket_dx[status - 1u]);
            rocket_y[x] =
                (uint8_t)((int8_t)rocket_y[x] + rocket_dy[status - 1u]);
        }
    }
...
void vertblkd(void)
{
    ...
    chopper_col =
        (uint8_t)((uint8_t)(((M2PL_HW | M3PL_HW) & 0x03u) | P0PL_HW | P1PL_HW)
                      << 4 |
                  P0PF_HW | P1PF_HW);

    /* Compute ROBOT.COL: collisions involving players 2+3 (robot) */
    robot_col = (uint8_t)(((M0PL_HW | M1PL_HW) & 0x0Cu) | P2PL_HW | P2PF_HW |
                          P3PL_HW | P3PF_HW);
