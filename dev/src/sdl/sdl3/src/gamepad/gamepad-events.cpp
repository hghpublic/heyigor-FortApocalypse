...
SDL_AppResult SDL_AppInit(void** appstate, int argc, char* argv[])
{
    ...
    SDL_SetAppMetadata("Example Input Gamepad Events", "1.0",
    ...
    if (!SDL_CreateWindowAndRenderer("examples/input/gamepad-events", 640, 480,
...
SDL_AppResult SDL_AppEvent(void* appstate, SDL_Event* event)
{
    ...
    else if (event->type == SDL_EVENT_GAMEPAD_ADDED)
    {
        ...
        if (!gamepad)
        {
            add_message(which, "Gamepad #%u add, but not opened: %s",
                        (unsigned int)which, SDL_GetError());
        }
        else
        {
            char* mapping = SDL_GetGamepadMapping(gamepad);
            add_message(which, "Gamepad #%u ('%s') added", (unsigned int)which,
                        SDL_GetGamepadName(gamepad));
            if (mapping)
            {
                add_message(which, "Gamepad #%u mapping: %s",
                            (unsigned int)which, mapping);
                SDL_free(mapping);
            }
        }
    }
    ...
    else if (event->type == SDL_EVENT_GAMEPAD_AXIS_MOTION)
    {
        ...
        if (now >= axis_motion_cooldown_time)
        {
            ...
            add_message(
                which, "Gamepad #%u axis %s -> %d", (unsigned int)which,
             ...
             (event->type == SDL_EVENT_GAMEPAD_BUTTON_DOWN))
    {
        ...
        add_message(which, "Gamepad #%u button %s -> %s", (unsigned int)which,
    ...
    else if (event->type == SDL_EVENT_JOYSTICK_BATTERY_UPDATED)
    {
        ...
        { /* this is only reported for joysticks, so make sure this joystick is
            ...
            add_message(which, "Gamepad #%u battery -> %s - %d%%",
                        (unsigned int)which,
...
SDL_AppResult SDL_AppIterate(void* appstate)
{
    ...
    while (msg)
    {
        ...
        const float life_percent =
            ((float)(now - msg->start_ticks)) / msg_lifetime;
        ...
        x = (((float)winw) -
             (SDL_strlen(msg->str) * SDL_DEBUG_TEXT_FONT_CHARACTER_SIZE)) /
            2.0f;
        y = ((float)winh) * life_percent;
        if ((prev_y != 0.0f) &&
            ((prev_y - y) < ((float)SDL_DEBUG_TEXT_FONT_CHARACTER_SIZE)))
        {
            msg->start_ticks = now;
        ...
        SDL_SetRenderDrawColor(
            renderer, msg->color.r, msg->color.g, msg->color.b,
