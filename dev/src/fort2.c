 ...
 *              chopper_status, robot_status, r_status, sx, sy, sx_f, sy_f,
 *              bak2_color, s1_1_val, s1_2_val, s2_val, s3_val, s4_val, s5_val, s6_val,
 *              pod_num, pod_com, pod_status[], pod_x[], pod_y[], pod_dx[],
 *              pod_temp1[], pod_temp2[], tank_status[], tank_x[], tank_y[], tank_dx[],
 *              tank_temp[], tank_start_x[], tank_start_y[], tank_spd, tank_speed,
 *              cm_status[], cm_x[], cm_y[], cm_time[], cm_temp[],
 *              missile_spd, missile_speed, rocket_status[], rocket_x[], rocket_temp[],
 *              rocket_tempx[], rocket_tempy[], bonus1, bonus2, chop_left, demo_status
 *   Functions: compute_map_adr(), pos_it(), inc_score(), main_loop(),
 ...
 *               pod_status[], pod_x[], pod_y[], pod_dx[], pod_temp1[], pod_temp2[],
 *               demo_status
 *   Constants:  PLAY_SCRN, CHR_SET1, CHR_SET2,
 *               CHR_EXP, CHR_EXP_WALL, CHR_MISS_LEFT, CHR_MISS_RIGHT
 *   HW aliases: REG(); FRAME_HW=FRAME, CONSOL_HW=CONSOL, TRIG0_HW=TRIG0,
 *               KBCODE_HW=KBCODE, SKSTAT_HW=SKSTAT, RANDOM_HW=RANDOM,
 *               AUDC1_HW..AUDC4_HW=AUDC1..AUDC4, SDLST_LO/HI=SDLST_L/H
 * ----------------------------------------------------------------------- */
...
/* Cruise missile direction status (also happens to equal STATUS_CRASH / 4 and 8) */
#define CM_LEFT   4u
#define CM_RIGHT  8u
...
static const uint8_t opt1[]  = { 'G','R','A','V','I','T','Y',' ','S','K','I','L','L', 0xFF };
static const uint8_t opt2[]  = { 'P','I','L','O','T',' ','S','K','I','L','L', 0xFF };
static const uint8_t opt3[]  = { 'R','O','B','O',' ','P','I','L','O','T','S', 0xFF };
 ...
 * ----------------------------------------------------------------------- */
static const uint8_t pod_chr[8] = {
...
void check_options(void)
{
    ...
    if (con == 3) {                         /* OPTION button: advance opt_num */
        uint8_t n = (uint8_t)(opt_num + 1u);
        if (n >= 3) n = 0;
        opt_num = n;
    }

    if (con == 5) {                         /* SELECT button: increment current setting */
        switch (opt_num) {
        case 0:                             /* gravity skill */
            { uint8_t v = (uint8_t)(grav_skill + 1u); if (v >= 3) v = 0; grav_skill = v; }
            break;
        case 1:                             /* pilot skill */
            { uint8_t v = (uint8_t)(pilot_skill + 1u); if (v >= 3) v = 0; pilot_skill = v; }
            break;
        case 2:                             /* chops (robo pilots) */
            { uint8_t v = (uint8_t)(chops + 1u); if (v >= 3) v = 0; chops = v; }
            break;
        }
    }
...
{
    uint8_t con = CONSOL_HW;
    if (con == consol_flag) goto keyboard_check;  /* no change */
    consol_flag = con;
    tim6_val = 0;

    if (con == 6) {                 /* START button pressed */
        mode = START_MODE;
        demo_status = 1;
        return;
    }
    ...
    if (con == 3 || con == 5) {     /* OPTION or SELECT button */
        mode = OPTION_MODE;
        demo_status = 1;
        screen_off();
        opt_num = 0;
...
keyboard_check:
    if (SKSTAT_HW & 0x04) return;           /* bit2=1: no key available */
    if (KBCODE_HW != 0x21) return;          /* not SPACE ($21) */
    ...
    for (;;) {
        if (!(SKSTAT_HW & 0x04)) {          /* key available */
            if (KBCODE_HW == 0x21) break;   /* space: unpause */
        } else {
            if (CONSOL_HW != 7) break;      /* any console button */
            if (!TRIG0_HW) break;           /* trigger (0=pressed) */
            continue;
        }
    }
...
static void p_begin(uint8_t xi)
{
    for (;;) {
        uint8_t rx;
        do { rx = RANDOM_HW; } while (rx < 50 || rx >= (256u - 50u));
        pod_x[xi] = rx;
        uint8_t ry;
        do { ry = RANDOM_HW; } while (ry >= 40);
        pod_y[xi] = ry;

        uint8_t *mp = pod_map_ptr(xi);
        if (mp[0] == 0 && mp[1] == 0) break;  /* empty: use this position */
    }
...
static void m_begin(uint8_t xi)
{
    ...
    if ((int8_t)(chop_x - tank_x[xi]) < 0) dir = CM_LEFT;
    cm_status[xi] = dir;
    cm_temp[xi] = 0;
    cm_time[xi] = 20;
    s6_val = 1;
...
{
    uint8_t *mp = cm_map_ptr(xi);
    if (mp[0] == CHR_EXP) return m_col2(xi);
    return 0;
}
...
static int m_draw_and_check(uint8_t xi)
{
    ...
    if (check_chr_byte(saved)) return m_col2(xi);
    return 0;
}
...
static void mm1(void)
{
    ...
    for (int xi = MAX_TANKS - 1; xi >= 0; xi--) {
        if (cm_status[xi] != STATUS_OFF) return;
    }
    AUDC4_HW = 0;
    s6_val = 0;
}
...
{
    if (mode != HYPERSPACE_MODE) return;
...
void move_tanks(void)
{
    ...
    tank_spd--;
    if (tank_spd != 0) goto mt2;
    tank_spd = tank_speed;

    for (int xi = MAX_TANKS - 1; xi >= 0; xi--) {
        uint8_t st = tank_status[xi];
        if (st == STATUS_OFF) goto next_tank;
        ...
        if (st == STATUS_CRASH) goto next_tank;
        ...
        compute_map_adr();
        {
            ...
            if (collision) {
                ...
                mp[0] = CHR_EXP; mp[1] = CHR_EXP; mp[2] = CHR_EXP;
                tim5_val = 10;
                goto next_tank;
            }
...
draw_tank:
        ...
        compute_map_adr();
        {
            ...
            if (check_tank_col((uint8_t)xi, 0)) goto draw_tank;
            if (check_tank_col((uint8_t)xi, 2)) goto draw_tank;
            ...
            uint8_t indicator = (uint8_t)
                ((chop_x >= tank_x[xi]) ? 0xEFu : 0xF0u);
            mp[-256 + 1] = indicator;
        }
    ...
    for (int xi = MAX_TANKS - 1; xi >= 0; xi--) {
        if (tank_status[xi] == STATUS_CRASH) {
            ...
            compute_map_adr();
            {
                uint8_t *mp = adr1_ptr();
                uint8_t tt = (uint8_t)((uint8_t)xi * 3u);
                for (uint8_t y = 0; y < 3u; y++) mp[y] = tank_temp[tt + y];
                mp[-256 + 1] = 0;
            }
            inc_score(0x50, 0x02);
            pos_tank((uint8_t)xi);
...
mt2_respawn:
    ...
    for (int xi = MAX_TANKS - 1; xi >= 0; xi--) {
        if (tank_status[xi] != STATUS_OFF) continue;
        /* Chopper must be near tank start row */
        if (chop_y < 3) continue;
        if ((uint8_t)(chop_y - 3u) < tank_start_y[xi]) continue;
        ...
        compute_map_adr();
        {
            ...
            uint8_t clear = 1;
            for (int y = 12; y >= 0; y--) {
                if (mp[y] & 0x80u) { clear = 0; break; }
            }
            if (clear) tank_status[xi] = STATUS_BEGIN;
        }
    }
}
...
void screen_off(void)
{
    ...
    if (r_status == STATUS_CRASH) r_status = STATUS_OFF;
    ...
    s1_1_val = 0; s2_val = 0; s3_val = 0;
    s4_val = 0;   s5_val = 0; s6_val = 0;
    bak2_color = 0;
    s1_2_val = 20;
    ...
    for (int ri = 2; ri >= 0; ri--) {
        if (rocket_status[ri] == 7) {   /* STATUS == 7 = EXP for rockets */
...
void wait_frame(uint8_t n)
{
    ...
    do {
        uint8_t f = FRAME_HW;
        while (f == FRAME_HW) {}   /* spin until frame counter advances */
        read_user();
        if (mode != saved) {
