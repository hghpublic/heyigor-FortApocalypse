...
static const uint8_t demo_stick[108] = {
    ...
    0x09,0x0B,0x0A,0x0A,0x0B,0x09,0x0B,0x0A,
    0x0B,0x09,0x0B,0x0A,0x0A,0x0A,0x0A,0x0B,
    0x0B,0x09,0x09,0x0B,0x0B,0x0A,0x09,0x0D,
    0x09,0x0B,0x0A,0x0A,0x0A,0x0A,0x0A,0x0A,
    0x0B,0x09,0x09,0x09,0x0A,0x0E,0x06,0x07,
    0x07,0x07,0x05,0x05,0x05,0x05,0x07,0x06,
    0x06,0x07,0x05,0x06,0x06,0x07,0x05,0x05,
    0x07,0x06,0x05,0x07,0x07,0x07,0x06,0x06,
    0x06,0x06,0x06,0x07,0x07,0x06,0x05,0x05,
    0x07,0x06,0x06,0x07,0x05,0x05,0x07,0x06,
    0x06,0x06,0x07,0x05,0x05,0x07,0x06,0x06,
    0x07,0x06,0x09,0x09,
};
...
void hover(void) {
    ...
    if (a >= 4u && a < 14u) return;
    if (a < 8u) { chopper_angle += 2u; chopper_angle += 2u; }
    else        { chopper_angle -= 2u; chopper_angle -= 2u; }
}
...
static void do_stick(void) {
    ...
    if (fuel_status == STATUS_EMPTY) s1_2_val = 60u;

    /* RIGHT */
    if (!(stick & JOY_RIGHT)) {
        s1_2_val = 17u;
        if (chopper_angle >= 14u || !(frame & 1u)) chopper_x++;
        if (!(frame & 3u)) { chopper_angle += 2u; chopper_angle += 2u; }
    }

    /* LEFT */
    if (!(stick & JOY_LEFT)) {
        s1_2_val = 17u;
        if (chopper_angle < 4u || !(frame & 1u)) chopper_x--;
        if (!(frame & 3u)) { chopper_angle -= 2u; chopper_angle -= 2u; }
    }
    ...
    if (chopper_angle & 0x80u) chopper_angle = 0u;
    if (chopper_angle >= 18u)  chopper_angle = 16u;
...
void read_stick(void) {
    if (chopper_status == STATUS_OFF || chopper_status == STATUS_CRASH) return;
    do_stick();
}
...
void draw_map(void) {
    ...
    if (chopper_x > MIN_RIGHT) {
        chopper_x = MIN_RIGHT;
        if (sx >= (uint8_t)(0xD8u + 1u)) sx = (uint8_t)(1u + 1u);
        sx_f--;
        if ((sx_f & 3u) == 3u) sx++;
...
void do_laser_1(void) {
    ...
    if (laser_status == STATUS_OFF) {
        ...
        for (int i = 31; i >= 0; i--) LASERS_1[i] = 0u;
        for (int i = 7;  i >= 0; i--) LASER_3[i]  = 0u;
        return;
    }
    ...
    if (tim1_val == 0u) {
        ...
        for (uint8_t i = 0u; i < 32u; i++) LASERS_1[i] = laser_shapes[i];
        for (uint8_t i = 0u; i <  8u; i++) LASER_3[i]  = laser_shapes[24u + i];
    }
}
...
void do_laser_2(void) {
    ...
    if (tim2_val == 0u) {
        for (uint8_t i = 0u; i < 32u; i++) LASERS_2[i] = laser_shapes[i];
        for (uint8_t i = 0u; i <  8u; i++) LASER_3[i]  = laser_shapes[16u + i];
    }
}

void do_blocks(void) {
    if (FRAME_HW & 0x7Fu) return;
    /* Clear BLOCK.1 (32 bytes covers BLOCK.1-4) */
    for (int i = 31; i >= 0; i--) BLOCK_1[i] = 0u;
    /* Randomly fill each block's 8 bytes */
    if (!(RANDOM_HW & 0x80u)) for (int i = 7; i >= 0; i--) BLOCK_1[i] = 0x55u;
    if (!(RANDOM_HW & 0x80u)) for (int i = 7; i >= 0; i--) BLOCK_2[i] = 0x55u;
    if (!(RANDOM_HW & 0x80u)) for (int i = 7; i >= 0; i--) BLOCK_3[i] = 0x55u;
    if (!(RANDOM_HW & 0x80u)) for (int i = 7; i >= 0; i--) BLOCK_4[i] = 0x55u;
}

void do_elevator(void) {
    elevator_tim--;
    if (elevator_tim) return;

    elevator_tim = elevator_spd;

    /* Clear BLOCK.5 (32 bytes covers BLOCK.5-8) */
    for (int i = 31; i >= 0; i--) BLOCK_5[i] = 0u;
    ...
    for (int i = 7; i >= 0; i--) target[i] = 0x55u;
}

void do_exp(void) {
    /* Animate explosion shapes using EXP_SHAPE masked with random */
    for (int i = 7; i >= 0; i--) {
        uint8_t v = (uint8_t)(EXP_SHAPE[i] & RANDOM_HW);
        EXPLOSION_PTR[i]  = v;
        EXPLOSION2_PTR[i] = v;
    }
...
void do_numbers(void) {
    if (mode == NEW_PLAYER_MODE || mode == GAME_OVER_MODE) return;
    do_n();
}
...
void ddig(uint8_t val) {
    ...
    if (!s_flg) {
        if (y & 0xF0u) s_flg = 1u;
        else           y |= 0xA0u;  /* mark hi nibble as blank (0xA) */
    } else {
        s_flg = 1u;
    }
    ...
    if (!s_flg) {
        if (y & 0x0Fu) s_flg = 1u;
        else           y |= 0x0Au;  /* mark lo nibble as blank (0xA) */
    } else {
        s_flg = 1u;
    }
